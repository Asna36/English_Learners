...
const Nouns = () => {
    ...
    try {
      // Check if we have a cached greeting for this username
      const cachedGreeting = localStorage.getItem(`greeting_${name}`);
      if (cachedGreeting) {
        console.log('Using cached greeting');
        setGreetingAudio(cachedGreeting);
        return;
      }

      console.log('Generating new greeting, attempt:', retryCount + 1);
      // Using Bill's voice with Turbo model
      const response = await fetch('https://api.elevenlabs.io/v1/text-to-speech/pqHfZKP75CvOlQylNhV4', {
        method: 'POST',
        headers: {
          'Accept': 'audio/mpeg',
          'Content-Type': 'application/json',
          'xi-api-key': 'sk_c94cbe12b86e58f62e95ba8fd2a63c57cf2aba1b67cdd0a4'
        },
        body: JSON.stringify({
          text: `Hello!! ${name}!!`,
      ...
      localStorage.setItem(`greeting_${name}`, audioUrl);
